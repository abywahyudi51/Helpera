...
export default function Riwayat() {
    ...
    const unsubscribe = onSnapshot(q, (snapshot) => {
      ...
      data.forEach(async (order: any) => {
        if (order.status === 'Selesai' || order.status === 'Batal' || order.status === 'Dibatalkan') {
          ...
          if (timeRef) {
            const statusTime =
              timeRef instanceof Timestamp
                ? timeRef.toDate()
                : new Date(timeRef.seconds * 1000);

            const diffMinutes =
              (now.getTime() - statusTime.getTime()) / 1000 / 60;

            if (diffMinutes > 10) {
              try {
                await deleteDoc(doc(db, 'bookings', order.id));
              } catch (e) {}
            }
          }
        }
  ...
  const filteredOrders = orders.filter((order) => {
    ...
    if (activeTab === 'batal')
      return status === 'batal' || status === 'dibatalkan' || status === 'cancelled';
    return true;
  });
  ...
  if (loading)
    return (
      <div className="min-h-screen flex justify-center items-center text-gray-400 bg-gray-50">
        Memuat data...
      </div>
    );
        ...
        <div className="w-full md:w-1/4 md:sticky md:top-24 z-10 mb-4 md:mb-0">
          <div className="mb-4 md:mb-6 text-center md:text-left">
            ...
            <p className="text-xs md:text-sm text-gray-500 font-medium mt-1">
              Kelola semua pesanan jasamu di sini.
            </p>
          </div>
          ...
          <div className="flex md:flex-col gap-2 overflow-x-auto no-scrollbar pb-2 md:pb-0">
            {['semua', 'proses', 'selesai', 'batal'].map((tab) => (
              ...
              >
                ...
                {tab}
                {activeTab === tab && (
                  <div className="ml-auto w-1.5 h-1.5 rounded-full bg-[#0F3D85] hidden md:block"></div>
                )}
              </button>
            ))}
          </div>
              ...
              >
                ...
                <div className="flex gap-3 md:gap-6 items-start">
                  ...
                  <div className="flex-1 min-w-0">
                    <h4
                      className={`font-bold text-sm md:text-xl mb-0.5 md:mb-1 truncate ${isCancelled ? 'text-gray-500 line-through' : 'text-gray-900'}`}
                    >
                      {order.serviceName}
                    </h4>
                    ...
                    <div className="flex items-center gap-1 text-[10px] md:text-sm text-gray-400 mb-1 md:mb-2">
                      ...
                      <span className="truncate">{order.location || 'Lokasi GPS'}</span>
                    </div>
                    ...
                    <>
                      ...
                      <div className="flex gap-2">
                        {order.status === 'Proses' && (
                          <button
                            onClick={() => openCancelModal(order.id)}
                            className="px-3 py-1.5 rounded-lg border border-red-100 text-red-500 text-[10px] font-bold hover:bg-red-50"
                          >
                            Batal
                          </button>
                        )}
                        ...
                        <Link
                          to={`/chat/${order.id}`}
                          className="px-4 py-1.5 bg-[#0F3D85] text-white rounded-lg text-[10px] font-bold shadow-sm flex items-center gap-1"
                        >
